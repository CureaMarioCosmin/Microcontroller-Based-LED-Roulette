...
void shiftOut(uint8_t data) {
  for (int i = 7; i >= 0; i--) {
      ...
      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_1, (data & (1 << i)) ? GPIO_PIN_SET : GPIO_PIN_RESET); 
      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_2, GPIO_PIN_SET);   
  }
}

void displayLED(uint8_t index) {
  uint8_t data[3] = {0};
...
uint8_t selectBet() {
  ...
  while (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_5) == GPIO_PIN_SET) { 
      if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_4) == GPIO_PIN_RESET) { 
          selection = (selection + 1) % 24;
          displayLED(selection);
          HAL_Delay(200); 
      }
  }
...
void rouletteGame(uint8_t bet) {
    const uint8_t totalLEDs = 24;
    ...
    int totalSteps = 60 + rand() % 41;

    for (int i = 0; i < totalSteps; i++) {
        displayLED(current);
        HAL_Delay(delayTime);

        current = (current + 1) % totalLEDs;

        if (i > totalSteps - 20) delayTime += 5;
    }
   ...
   if( HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_RESET ) {     // PORTC.13 ==0 means button pressed
    ...
    if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_6) == GPIO_PIN_RESET) {
      ...
      if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_6) == GPIO_PIN_RESET) {
          ...
          printf("Va rugam alegeti o pozitie pe care doriti sa pariati!\n\r");
          uint8_t bet = selectBet();
          rouletteGame(bet);
          break;
      }
  }
